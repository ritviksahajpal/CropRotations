...
TAG                    = 'WCB_'                                              
list_states            = 'WCB.txt' # Name of the file which contains the list of states (or watersheds/counties etc) to process
...
prev_first_yr          = 2010  # used in case we need to have missing CDL for a year
first_yr               = 2010
last_yr                = 2012
base_yr                = 2012  # This is the raster which will be used to fill in the gaps
list_min_area_rot      = [0.0,0.15,0.3,0.45,0.6,0.75,0.8,0.9,1.0]  # KEY PARAM: The minimum area of the state which should be covered by our derived crop rotations to stop finding out new rotations
list_min_perc_of_rot   = [0.0,0.005,0.01,0.05,0.1,0.2,0.6,1.0] # KEY PARAM: The minimum rate of increase of selecting new crop rotations before our algorithm is stopped
...
def computeCropRotations(state, out_dir, crp_rot_files, local_first_yr, local_last_yr, min_area_rot, min_perc_of_rot, first_loop):
    ...
    # Reclassify the existing rotations so that they do not overlap with the otehr CDL ID's.
    recl_cur_to_higher   = open(out_dir+os.sep+'recl_cur_to_higher.csv', 'wb')
    num_rot_reclassified = 0
    for i in range(len(cdl_comb)):                                                                            
        if(cdl_comb[i][yrs_in_rot+1]>0):                                                                                                                                                                 
            if(cdl_comb[i][yrs_in_rot+2]<max_rot_id):
                num_rot_reclassified += 1
                max_rot_id += 1
    ...
    for i in range(len(cdl_comb)):
        if(cdl_comb[i][yrs_in_rot+1]>0):
            # TODO: No longer need to call synchronize rotation
...
def initiate(list_min_area_rot, list_min_rate_increase, local_first_yr, local_last_yr, first_loop):
    ...
    if(bool_merge_ras_files and first_loop):    
...
# @return: 
###############################################################################    
# exp_file contains the distilled output results
exp_file  = open(base_dir+os.sep+'output'+os.sep+TAG+EXPR_FILE+'.csv','a+')
...
for a in list_min_area_rot:
    for b in list_min_perc_of_rot:
        ...
        tag            = TAG+date+'_'+str(a)+'_'+str(b)
        ROTATION_STATS = 'rot_stats_'+tag+'.csv'
        READABLE_ROTS  = 'human_readable_rotations_'+date+'.csv'
        seq = []

        output_dir     = base_dir+os.sep+'output_oct2013'+os.sep+tag # contains all the output files
        ...
        try:
            ...
            print 'Existing rotations file is missing.'
