...
# Create a prefix with the current date (Day_Month_Year)
...
HUMAN_READABLE_ROTATIONS = 'human_readable_rotations_'+date+'.csv'
...
# min_rate_increase: The minimum rate of increase of selecting new crop rotations before our algorithm is stopped
# remove_urban_and_wtlnd: Should be false if you want to create a wall to wall product
#######################################################################
base_dir           = 'C:\\Users\\rs\\Documents\\PhD\\Projects\\CropIntensity' # base_dir contains all the input and output files
...
list_states        = 'test.txt' # Name of the file which contains the list of states (or watersheds/counties etc) to process
tag                = date+'_mw_2008_2011_2'
...
#min_area_rot       = numpy.arange(0.5,1.05,0.05)  # KEY PARAM: The minimum area of the state which should be covered by our derived crop rotations to stop finding out new rotations
#min_rate_increase  = numpy.arange(0,0.035,0.005) # KEY PARAM: The minimum rate of increase of selecting new crop rotations before our algorithm is stopped
min_area_rot = 0.7
min_rate_increase = 0.005
...
# @param first_yr: first year of CDL
# @param last_yr: last year of CDL 
...
def computeCropRotations(crp_rot_files, first_yr, last_yr):    
    ...
    prev_perc_land_in_rot = 0.0
    rate_of_increase = 100.0
    cur_index = 0
    
    while(rate_of_increase>min_rate_increase and cur_perc_land_in_rot<min_area_rot):
...
try:
    if(len(to_merge_files)>1):
        arcpy.Merge_management(to_merge_files, merged_states_ras)
    else:
        arcpy.Rename_management(to_merge_files, merged_states_ras)
except:
    logger.info(arcpy.GetMessages())
...
human_readable_rot = open(output_dir+os.sep+HUMAN_READABLE_ROTATIONS,'wb')
for str_row in prev_rot_file:
    line = []
    row = [int(x) for x in str_row]
    line.append(row[0])
